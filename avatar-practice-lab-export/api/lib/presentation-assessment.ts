...
import type { PresentationData } from "./ppt-parser.js";
import { trackOpenAIUsage } from "../utils/api-usage-tracker.js";

const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });

export interface PresentationFeedbackDimension {
  name: string;
  score: number;
  maxScore: number;
  feedback: string;
...
export interface PresentationFeedback {
  overallScore: number;
  communication: PresentationFeedbackDimension;
  delivery: PresentationFeedbackDimension;
  subjectMatter: PresentationFeedbackDimension;
  slideCoverage: SlideCoverage[];
  strengths: string[];
  improvements: string[];
  summary: string;
}

// Skill-based assessment for Skill ID 104: Effective Presentation & Communication in Groups
export interface SkillDimensionAssessment {
...
export interface PresentationSkillAssessment {
  skillId: number;
  skillName: string;
  overallScore: number;
  dimensions: {
...
// Document analysis for uploaded presentation files - detailed slide-by-slide review
...
export interface DocumentAnalysisFeedback {
  overallScore: number;
...
export async function generatePresentationFeedback(
  ...
  const slideSummary = presentationData.slides?.slice(0, maxSlides).map(s => 
    `Slide ${s.slideNumber}: "${s.title}" - Key points: ${s.bulletPoints?.join(", ") || "No bullet points"}`
  ).join("\n") || "No slides available";
...
Provide specific evidence from the transcript for all feedback.`;
  ...
  try {
    ...
    return JSON.parse(msg) as PresentationFeedback;
  } catch (error: any) {
 ...
 */
export async function generatePresentationSkillAssessment(
  transcript: string,
  presentationData: PresentationData,
  topic: string
): Promise<PresentationSkillAssessment | null> {
...
Calculate the overall score as the weighted average of all dimensions.`;
  ...
  try {
    ...
    return JSON.parse(msg) as PresentationSkillAssessment;
  } catch (error: any) {
...
export async function generateDocumentAnalysis(
  presentationData: PresentationData,
  topic: string
): Promise<DocumentAnalysisFeedback | null> {
...
Output ONLY valid JSON matching the schema.`,
    ...
    return JSON.parse(msg) as DocumentAnalysisFeedback;
  } catch (error: any) {
