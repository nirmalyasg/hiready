...
type RoleKit = typeof roleKits.$inferSelect;
type CaseTemplate = typeof caseTemplates.$inferSelect;
type CodingExercise = typeof codingExercises.$inferSelect;
...
interface ExerciseContext {
  exerciseType: "case_study" | "coding_lab";
  templateId?: number;
...
function calculateSkillOverlap(
  ...
  if (exerciseSkills.length === 0 || jobSkills.length === 0) return 0;
...
function checkRoleTypeMatch(
  ...
  for (const [category, patterns] of Object.entries(rolePatterns)) {
    ...
    const templateMatchesCategory = templateTags.some(tag => 
      patterns.some(p => tag.toLowerCase().includes(p))
    ) || patterns.some(p => roleKitDomain.includes(p) || roleKitName.includes(p));
    
    if (templateMatchesCategory) return true;
  }
...
function checkDomainMatch(
  ...
  if (!roleKit?.domain) return false;
...
export async function getAutoLinkSuggestion(
  userId: string,
  ...
  let codingExercise: CodingExercise | null = null;
  let roleKit: RoleKit | null = null;
  ...
  const exerciseSkills: string[] = [];
  if (template?.evaluationFocus) exerciseSkills.push(...template.evaluationFocus);
  if (template?.tags) exerciseSkills.push(...template.tags);
  if (template?.name) exerciseSkills.push(...extractKeywords(template.name));
  if (template?.caseType) exerciseSkills.push(template.caseType.replace(/_/g, " "));
  ...
  if (roleKit?.skillsFocus) exerciseSkills.push(...roleKit.skillsFocus);
  if (roleKit?.coreCompetencies) exerciseSkills.push(...roleKit.coreCompetencies);
  if (roleKit?.trackTags) exerciseSkills.push(...roleKit.trackTags);
  if (roleKit?.name) exerciseSkills.push(...extractKeywords(roleKit.name));
  if (roleKit?.domain) exerciseSkills.push(roleKit.domain.toLowerCase());
  if (codingExercise?.tags) exerciseSkills.push(...codingExercise.tags);
  if (codingExercise?.name) exerciseSkills.push(...extractKeywords(codingExercise.name));
  if (codingExercise?.language) exerciseSkills.push(codingExercise.language.toLowerCase());
  if (codingExercise?.activityType) exerciseSkills.push(codingExercise.activityType.replace(/_/g, " "));
  ...
  const recentSessionsRaw = await db
    ...
    .where(
      and(
        ...
        isNotNull(exerciseSessions.jobTargetId)
      )
    )
  ...
  const scoredJobs: MatchResult[] = userJobs.map(job => {
    const jobSkills: string[] = [];
    if (job.jdParsed?.requiredSkills) jobSkills.push(...job.jdParsed.requiredSkills);
    if (job.jdParsed?.preferredSkills) jobSkills.push(...job.jdParsed.preferredSkills);
    if (job.jdParsed?.focusAreas) jobSkills.push(...job.jdParsed.focusAreas);
    if (job.roleTitle) jobSkills.push(...extractKeywords(job.roleTitle));
    if (job.jdText) {
    ...
    if (job.status === "interview") score += 8;
    if (job.status === "applied") score += 5;
    
    if (userJobs.length === 1) score += 10;
    ...
    if (roleTypeMatch) rationalePoints.push(`Matches ${job.roleTitle}`);
    if (domainMatch) rationalePoints.push("Domain alignment");
    if (skillOverlap > 0.1) rationalePoints.push(`${Math.round(skillOverlap * 100)}% skill overlap`);
    if (recentActivity) rationalePoints.push("Recently practiced");
    if (job.status === "interview") rationalePoints.push("Active interview process");
    if (job.status === "applied") rationalePoints.push("Applied");
...
export async function getRankedJobSuggestions(
  userId: string,
  context: ExerciseContext,
  limit: number = 3
): Promise<MatchResult[]> {
