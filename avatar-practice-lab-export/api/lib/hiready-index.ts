...
function normalizeInterviewType(type: string): string {
  return type.toLowerCase().replace(/_interview$/, '').replace(/-/g, '_');
}

function getTypeWeight(interviewType: string): number {
...
function getReadinessLevel(score: number): "not_ready" | "developing" | "ready" | "strong" | "exceptional" {
  const clampedScore = Math.min(100, Math.max(0, score));
  if (clampedScore >= 85) return "exceptional";
  if (clampedScore >= 70) return "strong";
  if (clampedScore >= 55) return "ready";
  if (clampedScore >= 40) return "developing";
  return "not_ready";
}

function normalizeScore(score: number, maxScore: number = 5): number {
  const normalized = Math.round((score / maxScore) * 100);
  return Math.min(100, Math.max(0, normalized));
}

export async function calculateConsolidatedHireadyIndex(
  userId: string,
  options: {
    jobTargetId?: string;
    roleKitId?: number;
...
): Promise<HireadyIndexResult | null> {
  ...
  const sessionsQuery = db
    ...
    .where(
      and(
        ...
        jobTargetId ? eq(interviewConfigs.jobTargetId, jobTargetId) : undefined,
        roleKitId ? eq(interviewConfigs.roleKitId, roleKitId) : undefined
      )
    )
  ...
  const dimensionAggregates: Record<string, { totalScore: number; count: number; evidence: string[]; maxScore: number; weightedSum: number; totalWeight: number }> = {};
  const sessionBreakdown: SessionBreakdown[] = [];
  const completedInterviewTypes = new Set<string>();
  ...
  for (const { session, config, analysis } of sessions) {
    ...
    if (analysis.dimensionScores && Array.isArray(analysis.dimensionScores)) {
      ...
      const avgSessionScore = analysis.dimensionScores.reduce((sum: number, d: any) => sum + d.score, 0) / 
    ...
    if (analysis.strengths && Array.isArray(analysis.strengths)) {
      allStrengths.push(...(analysis.strengths as string[]).slice(0, 2));
    }
    if (analysis.improvements && Array.isArray(analysis.improvements)) {
      allImprovements.push(...(analysis.improvements as string[]).slice(0, 2));
    }
  }

  const dimensionScores: DimensionScore[] = Object.entries(dimensionAggregates).map(([dimension, data]) => {
  ...
  let roleContext: HireadyIndexResult["roleContext"];
...
export async function saveHireadyIndex(
  userId: string,
  indexData: HireadyIndexResult,
  options: {
    jobTargetId?: string;
    roleKitId?: number;
    employerJobId?: string;
...
export async function getPublicHireadyIndex(shareToken: string) {
  const [index] = await db
    .select()
    .from(hireadyRoleIndex)
    .where(
      and(
        eq(hireadyRoleIndex.shareToken, shareToken),
        eq(hireadyRoleIndex.isPublic, true)
      )
    )
    .limit(1);
