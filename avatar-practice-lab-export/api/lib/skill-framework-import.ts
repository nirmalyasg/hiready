...
interface SkillFrameworkData {
  ...
  scoring_model_logic: string;
}

export function parseCSV(csvContent: string): SkillFrameworkData[] {
  const lines = csvContent.split("\n");
  const headers = lines[0].split(",").map((h) => h.trim().toLowerCase().replace(/"/g, ""));
  
  const skills: SkillFrameworkData[] = [];
...
export async function importSkillFrameworks(pool: Pool, csvFilePath: string): Promise<{ updated: number; errors: string[] }> {
  const errors: string[] = [];
  let updated = 0;
...
export function parseDimensions(dimensionsString: string): string[] {
...
export interface SkillWithFramework {
  id: number;
  name: string;
  description: string;
  definition: string;
  ...
  scoringModelLogic: string | null;
}

export async function getSkillWithFramework(pool: Pool, skillId: number): Promise<SkillWithFramework | null> {
  const result = await pool.query(
...
export async function getSkillsForScenario(pool: Pool, scenarioId: number | null): Promise<SkillWithFramework[]> {
  ...
  try {
    const result = await pool.query(
      `SELECT s.id, s.name, s.description, s.definition,
...
export async function getSkillsForCustomScenario(pool: Pool, customScenarioId: number): Promise<SkillWithFramework[]> {
