...
interface CompanyBlueprintData {
  companyId: string;
  companyName: string;
  archetype: string;
  tier: string;
...
interface PracticeOption {
  id: string;
  type: "interview_round" | "coding_practice" | "case_study" | "behavioral" | "generic_interview";
...
function detectSeniority(roleTitle: string, experienceLevel?: string): string {
  const roleLower = roleTitle.toLowerCase();
...
export async function findCompanyBlueprint(
  companyName: string | null,
  roleTitle: string,
  experienceLevel?: string
): Promise<CompanyBlueprintData | null> {
  ...
  const companyResults = await db
    ...
    .from(companies)
    .where(
      sql`LOWER(${companies.name}) = ${companyNameClean} OR LOWER(${companies.name}) LIKE ${'%' + companyNameClean + '%'}`
    )
    .limit(1);
  ...
  let bestMatch = blueprints.find(
    b => b.roleCategory === roleCategory && b.seniority === seniority
  );
  ...
  return {
    ...
    blueprint: bestMatch ? {
      ...
      skillFocus: (bestMatch.skillFocus as string[]) || [],
      interviewRounds: (bestMatch.interviewRounds as InterviewRound[]) || [],
      notes: bestMatch.notes,
      rubricOverrides: bestMatch.rubricOverrides as {
        dimensions?: { key: string; weight: number }[];
        passingScore?: number;
        style?: string;
      } | null,
...
export function generatePracticeOptions(
  blueprintData: CompanyBlueprintData | null,
  jdParsed: {
    focusAreas?: string[];
  ...
  if (blueprintData?.blueprint) {
    ...
    const uniqueRoundTypes = new Map<string, InterviewRound>();
    ...
    roundEntries.forEach(([roundType, round], index) => {
      ...
      const isTechnical = round.focus.some(f => 
        ["coding", "algorithms", "dsa", "system-design", "machine-coding", "debugging", "technical"].includes(f)
      );
      const isBehavioral = round.focus.some(f => 
        ["behavioral", "leadership", "leadership-principles", "googleyness", "values", "culture-fit", "ownership"].includes(f)
      );
      
      let practiceType: PracticeOption["type"] = "interview_round";
...
export async function getCompanyAwarePracticeOptions(
  companyName: string | null,
  roleTitle: string,
  jdParsed: {
    focusAreas?: string[];
  ...
  companyData: CompanyBlueprintData | null;
}> {
  const companyData = await findCompanyBlueprint(
    companyName,
    roleTitle,
