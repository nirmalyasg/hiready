...
exerciseModeRouter.get("/case-templates", async (req: Request, res: Response) => {
  try {
...
exerciseModeRouter.get("/case-templates/:id", async (req: Request, res: Response) => {
  try {
    ...
    let roleKit: typeof roleKits.$inferSelect | null = null;
...
exerciseModeRouter.get("/coding-exercises", async (req: Request, res: Response) => {
  try {
...
exerciseModeRouter.get("/coding-exercises/:id", async (req: Request, res: Response) => {
  try {
    const id = parseInt(req.params.id);
...
exerciseModeRouter.get("/rubrics", async (req: Request, res: Response) => {
  try {
...
exerciseModeRouter.get("/rubrics/default/:exerciseType", async (req: Request, res: Response) => {
  try {
    ...
    const [rubric] = await db
      ...
      .where(and(
        eq(exerciseRubrics.exerciseType, exerciseType as "case_study" | "coding_lab"),
        eq(exerciseRubrics.isDefault, true)
      ));
...
exerciseModeRouter.post("/sessions", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const [session] = await db
      ...
      .values({
        userId,
        exerciseType: exerciseType as "case_study" | "coding_lab",
...
exerciseModeRouter.get("/sessions/:id", requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = req.user!.id;
    const id = parseInt(req.params.id);
    ...
    let caseTemplate: typeof caseTemplates.$inferSelect | null = null;
    let codingExercise: typeof codingExercises.$inferSelect | null = null;
    let roleKit: typeof roleKits.$inferSelect | null = null;
...
exerciseModeRouter.get("/sessions/by-uid/:uid", requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = req.user!.id;
...
exerciseModeRouter.patch("/sessions/:id/status", requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = req.user!.id;
    const id = parseInt(req.params.id);
    ...
    if (thinkingTimeUsed !== undefined) updateData.thinkingTimeUsed = thinkingTimeUsed;
    if (duration !== undefined) updateData.duration = duration;
    if (transcript !== undefined) updateData.transcript = transcript;
    if (userCodeSubmission !== undefined) updateData.userCodeSubmission = userCodeSubmission;
...
exerciseModeRouter.get("/sessions", requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = req.user!.id;
...
exerciseModeRouter.post("/sessions/:id/analyze", requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = req.user!.id;
    ...
    const [defaultRubric] = await db
      ...
      .from(exerciseRubrics)
      .where(and(
        eq(exerciseRubrics.exerciseType, session.exerciseType),
        eq(exerciseRubrics.isDefault, true)
      ));
...
exerciseModeRouter.get("/sessions/:id/analysis", requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = req.user!.id;
    const sessionId = parseInt(req.params.id);
...
exerciseModeRouter.get("/sessions/:id/interviewer-prompt", requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = req.user!.id;
    const sessionId = parseInt(req.params.id);
    ...
    if (session.exerciseType === "case_study" && session.caseTemplateId) {
      ...
      if (ct) {
        exerciseData = ct;
        const probingMap = ct.probingMap as { ifVague?: string[]; ifWrong?: string[]; ifStrong?: string[] } || {};
...
${(ce.probingQuestions || []).map((q: string, i: number) => `${i + 1}. ${q}`).join("\n")}

EXPECTED SIGNALS (what good answers include):
${(ce.expectedSignals || []).map((s: any) => `- ${s.signal} (${s.importance})`).join("\n")}

COMMON FAILURE MODES (watch for these):
${(ce.commonFailureModes || []).map((f: any) => `- ${f.mistake}: Feedback: ${f.feedback}`).join("\n")}
