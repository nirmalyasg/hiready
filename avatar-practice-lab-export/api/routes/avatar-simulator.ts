...
import {
  ...
  tones,
  personas,
  culturalStylePresets,
} from "../../shared/schema.js";
import {
  CULTURAL_STYLE_PRESETS,
  getCulturalPresetById,
  getDefaultCulturalPreset,
  getAllCulturalPresets,
  type CulturalStylePreset,
} from "../../shared/cultural-presets.js";
import { eq, sql, and, isNotNull } from "drizzle-orm";
export const avatarSimulator = Router();
import {
  requireAuth,
  requireCoachOrLearner,
  requireCoach,
  requireCurrentUser,
  requireAdmin,
...
import { 
  performSkillAssessmentForSession, 
  getSkillAssessmentsForSession 
...
const DISCUSSION_CATEGORIES: Record<string, { name: string; context: string; fallbackTopics: string[] }> = {
  "current-affairs": {
...
avatarSimulator.post("/end-sessions", async (req, res) => {
  ...
  try {
    ...
    if (listData.data && listData.data.sessions && listData.data.sessions.length > 0) {
      const stopPromises = listData.data.sessions.map(async (session: { session_id: string }) => {
        try {
...
avatarSimulator.post("/session/start", async (req, res) => {
  try {
    ...
    let validCulturalPresetId: string | null = null;
    if (culturalPresetId) {
...
avatarSimulator.get("/session/status", async (req, res) => {
  try {
    ...
    if (sessionId) {
      ...
      session = await storage.getHeygenSessionByHeygenId(heygenSessionId as string);
    } else {
      const authUserId = req.user?.id;
      if (authUserId) {
        const legacyUser = await storage.getLegacyUserByAuthUserId(authUserId);
...
avatarSimulator.post("/session/end", async (req, res) => {
  ...
  try {
    ...
    if (endedSession.actualDurationSec && endedSession.actualDurationSec > 0) {
      const authUserId = req.user?.id;
      await trackHeygenUsage(
        "streaming_session",
        endedSession.actualDurationSec * 1000, // convert to ms
        { 
          authUserId,
...
avatarSimulator.get("/get-scenarios", async (req, res) => {
  ...
  const parseParam = (param: any): string | null => {
  ...
  console.log(
    `GET /api/get-scenarios called with skillId: ${skillId}, role: ${role}, scenarioId: ${scenarioId}`,
  );
  ...
  try {
    ...
    res.status(500).json({
      success: false,
      message: (error as Error).message || "Database error",
      error: error,
    });
  }
});
...
avatarSimulator.post("/analyze-transcript", async (req, res) => {
  try {
    ...
    console.log(
      `Processing ${validMessages.length} valid messages for analysis`,
    ...
    try {
      const existingAnalysis = await pool.query(
        `SELECT * FROM analysis_results WHERE transcript_id = $1`,
        [transcript.id],
      );
...
Keep feedback specific, actionable, and evidence-based.`;
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert communication skills coach specializing in workplace scenarios. Provide clear, actionable feedback focused on practical improvement.",
        },
        {
          role: "user",
          content: prompt,
    ...
    const overallRating = parseInt(
      analysis.match(/Overall Rating: \[(\d+)/)?.[1] || "0",
    );
    const clarityScore = parseInt(
      analysis.match(/Clarity of Expression: \[(\d+)/)?.[1] || "0",
    );
    const professionalScore = parseInt(
      analysis.match(/Professional Demeanor: \[(\d+)/)?.[1] || "0",
    );
    const informationScore = parseInt(
      analysis.match(/Information Delivery: \[(\d+)/)?.[1] || "0",
    );
    const questionHandlingScore = parseInt(
      analysis.match(/Question Handling: \[(\d+)/)?.[1] || "0",
    );
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert communication coach specializing in workplace scenarios and interpersonal skills. Your task is to provide constructive, actionable feedback on conversation transcripts.",
        },
        {
          role: "user",
          content: prompt,
...
avatarSimulator.post("/generate-scenario", async (req, res) => {
  try {
    ...
    const aiResponse = await fetch(
      `${origin}/api/generate-scenario-instructions`,
      {
        method: "POST",
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert in creating realistic conversation scenarios for communication practice. Format your response as a JSON object with 'context' and 'instructions' fields.",
        },
        {
          role: "user",
          content: prompt,
  ...
  try {
    ...
    try {
      ...
      if (streamingResponse.ok) {
        ...
        console.warn(
          `streaming.create_token failed (${streamingResponse.status}):`,
          errorText,
        );
      }
    } catch (streamingError) {
      console.warn(
        "Exception in streaming.create_token endpoint:",
        streamingError,
      );
    }
    ...
    const fallbackResponse = await fetch(
      "https://api.heygen.com/v1/access_token",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!fallbackResponse.ok) {
      ...
      console.error(
        `HeyGen fallback API error (${fallbackResponse.status}):`,
        errorText,
      );
      return res
...
avatarSimulator.get("/get-session-recordings", async (req, res) => {
  ...
  try {
    ...
    const files = data.Contents.map((item) => {
      ...
      let speakerType: "user" | "avatar" | "combined" | "unknown" = "unknown";
      if (filename.startsWith("user_")) speakerType = "user";
      else if (filename.startsWith("avatar_")) speakerType = "avatar";
      else if (filename.startsWith("combined_")) speakerType = "combined";
...
avatarSimulator.post("/get-session-transcript-id", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      "SELECT transcript_id FROM streaming_sessions WHERE session_id = $1",
      [sessionId],
    );
...
avatarSimulator.get("/get-transcript", async (req, res) => {
  const transcriptId = req.query.id as string;
...
avatarSimulator.get("/skill-progress", async (req, res) => {
  try {
    ...
    const dimensionsBySkill: Record<number, Array<{dimension: string, avgScore: number, count: number}>> = {};
      ...
      messages: [
        {
          role: "system",
          content:
            "Structure the conversation transcript into a clear format with timestamp, speakers, and key points. For non-English text, provide both the original text and its English translation. Organize it professionally.",
        },
        {
          role: "user",
          content: transcript,
...
avatarSimulator.post("/save-onboarding", async (req, res) => {
  try {
    ...
    // Insert user data
    const result = await pool.query(
      `INSERT INTO users (email, role, industry, skill_level, goals, assessment_answers)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING id`,
      [
        data.email,
...
avatarSimulator.post("/save-streaming-session", async (req, res) => {
  try {
    ...
    if (!sessionResult.success) {
      throw new Error(
        sessionResult.error || "Failed to save streaming session",
      );
    }
  ...
  try {
    ...
    if (normalizedData.messages.length > 0) {
      console.log(
        ...
        JSON.stringify(normalizedData.messages[0]),
      );
    }
    ...
    return res.status(500).json({
      success: false,
      error: (error as Error)?.message || "Failed to save transcript",
    });
  }
});
...
avatarSimulator.post("/session-analysis", async (req, res) => {
  try {
    ...
    console.log(
      "Transcript messages:",
      JSON.stringify(transcriptMessagesResult, null, 2),
    );
    ...
    if (existingAnalysis?.length) {
      console.log(
        "Analysis already exists for transcriptId:",
        JSON.stringify(existingAnalysis, null, 2),
      );
      
      // Fetch skill assessments for existing analysis
      let skillAssessments: { summary: any[], dimensions: any[] } = { summary: [], dimensions: [] };
      ...
      if (sessionIdForAssessment) {
        try {
          ...
          if (skillAssessments.summary.length === 0 && skillAssessments.dimensions.length === 0) {
            ...
            const generatedResult = await performSkillAssessmentForSession(
              pool,
              sessionIdForAssessment,
              scenarioId,
              conversationText,
              customScenarioId,
              isImpromptuSession,
    ...
    const analysisResult = await generateSessionAnalysis(
      transcriptMessagesResult.messages,
    );
    console.log(
      "New Analysis result:",
    ...
    let roleplaySessionId: number | null = null;
    ...
    const [inserted] = await db
      ...
      .values({
        ...
        pronunciationSuggestions:
          analysisResult.feedback.pronunciation.suggestions,
      })
      .returning();

    let skillAssessments: any = { skillAssessments: [], personaFit: undefined };
    try {
      const conversationText = transcriptMessagesResult.messages
        .map((m: any) => `${m.speaker}: ${m.text}`)
        .join("\n");
      ...
      skillAssessments = await performSkillAssessmentForSession(
        pool,
        roleplaySessionId,
        transcript.scenarioId || null,
        conversationText,
        transcript.customScenarioId || null,
        isImpromptuSession,
        personaOverlay
      );
...
avatarSimulator.get("/session-analysis/:id", async (req, res) => {
  try {
    ...
    const result = await db
      ...
      .leftJoin(
        transcriptMessages,
        eq(roleplaySession.transcriptId, transcriptMessages.transcriptId),
      )
      .leftJoin(
        aiSessionAnalysis,
        eq(roleplaySession.id, aiSessionAnalysis.sessionId),
      )
      .where(eq(roleplaySession.id, roleplaySessionId));

    console.log(
      "✅ Session analysis fetched successfully",
    ...
    try {
      skillAssessments = await getSkillAssessmentsForSession(pool, parseInt(roleplaySessionId as string));
    } catch (skillError) {
...
avatarSimulator.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      //   file.mimetype,
      // );
      const uploaded = await uploadAudioFileToS3(
        fileBuffer,
        fileName,
        file.mimetype,
      );
...
avatarSimulator.post("/pull-heygen-avatars", async (req, res) => {
  try {
    ...
    function getGenderFromFullName(fullName: string) {
      if (!fullName) {
        return "unknown";
      }
    ...
    const mappedAvatars = data.data
      .filter(
        (a: any) =>
          a.is_public && a.status === "ACTIVE" && a.normal_preview !== null,
      )
      .map((a: any) => ({
        id: a.avatar_id,
        name: a.pose_name,
...
avatarSimulator.get("/list", async (req, res) => {
  try {
    ...
    if (ethnicity) filters.push(eq(avatars.ethnicity, ethnicity));
    if (gender) filters.push(eq(avatars.gender, gender));
    if (role) filters.push(eq(avatars.role, role));
  ...
  try {
    ...
    try {
      const conversationText = transcript
        .map((item: TranscriptItem) => `${item.speaker}: ${item.text}`)
        .join("\n");
      
      const transcriptRecord = await pool.query(
        `SELECT scenario_id, custom_scenario_id, context, session_config FROM transcripts WHERE id = $1`,
        [transcriptId]
      );
  ...
  try {
    ...
    const updatedData = await db
      ...
      .where(
        and(
          eq(
            assessmentRecommendedScenarios.assessmentAssignmentId,
            assignmentId,
          ),
          eq(assessmentRecommendedScenarios.scenarioId, scenarioId),
          eq(assessmentRecommendedScenarios.userId, userId),
        ),
      )
      .returning();
    return res.json({
...
avatarSimulator.get("/recommended-scenarios", async (req, res) => {
  try {
    ...
    if (isNaN(assignmentId)) {
      console.log(
        `[Assessment Report] Invalid assignment ID: ${req.query.assignmentId}`,
      );
      return res.status(400).json({
        success: false,
    ...
    console.log(
      ...
      JSON.stringify(assignment.assessment, null, 2),
    );
    if (!is360) {
      return res.status(400).json({
        success: false,
    ...
    const selfAssessment =
      await storage.getSelfAssessmentResponse(assignmentId);
    ...
    if (!selfAssessment) {
      console.log(
        `[Assessment Report] No report data generated for assignment: ${assignmentId}`,
      );
      return res.status(404).json({
        success: false,
    ...
    try {
        ...
        const scenariosWithCompletion = scenariosResult
          ...
          .map((s) => {
            ...
            return {
              ...
              isScenarioSessionCompleted:
                existingMatch?.isScenarioSessionCompleted ?? false,
            };
          });
        return res.json({
          success: true,
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a coach AI. Select 3 scenarios most relevant to the learner. Only return scenario IDs from the list provided.",
          },
          {
            role: "user",
      ...
      const pickedScenarios = scenariosResult.filter((s) =>
        picked.selected_scenarios.includes(s.id),
      );
      ...
      await db
        ...
        .values(
          picked.selected_scenarios.map((scenarioId: number) => ({
...
async function generateSessionAnalysis(transcript: TranscriptItem[]) {
  const conversationText = transcript
    ...
    messages: [
      {
        role: "system",
        content:
          "You are a formatter that outputs ONLY a single JSON object matching the provided JSON schema. No markdown, code fences, or extra text.",
      },
      {
        role: "user",
  ...
  if (!msg) throw new Error("No analysis generated");
...
async function saveTranscriptToDatabase(transcript: {
  ...
  sessionType?: "streaming_avatar" | "audio_roleplay";
  sessionConfig?: any;
}) {
  try {
    ...
    try {
      const existingRoleplay = await pool.query(
        `SELECT id, user_id FROM roleplay_session WHERE knowledge_id = $1 ORDER BY created_at DESC LIMIT 1`,
        [transcript.sessionId],
      );
    ...
    if (!roleplaySessionId) {
      try {
        ...
        const roleplayResult = await pool.query(
          `INSERT INTO roleplay_session (user_id, avatar_id, knowledge_id, duration, start_time)
           VALUES ($1, $2, $3, $4, NOW())
           RETURNING id`,
          [
            transcript.userId,
    ...
    try {
      const existingResult = await pool.query(
        `SELECT id FROM transcripts WHERE session_id = $1 LIMIT 1`,
        [roleplaySessionId],
      );
    ...
    if (existingTranscript) {
      ...
      await pool.query(
        `INSERT INTO transcripts (id, session_id, avatar_id, knowledge_id, context, instructions, duration, user_id, skill_id, scenario_id, custom_scenario_id, session_type, skill, scenario, session_config)
    ...
    if (transcript.messages?.length > 0 && finalTranscriptId) {
      ...
      await pool.query(
        `INSERT INTO transcript_messages (id, transcript_id, messages)
           VALUES ($1, $2, $3)
           ON CONFLICT (id) DO UPDATE SET messages = EXCLUDED.messages`,
        [messageId, finalTranscriptId, JSON.stringify(transcript.messages)],
      );
    }
    ...
    if (roleplaySessionId && finalTranscriptId) {
      try {
        await pool.query(
          `UPDATE roleplay_session SET transcript_id = $1 WHERE id = $2`,
          [finalTranscriptId, roleplaySessionId],
        );
...
export const getAllTranscripts = async (userId: number) => {
  try {
    const result = await pool.query(
      `
        SELECT 
    ...
    return {
      ...
      source: "memory",
      error: (error as Error).message || "Failed to get transcripts",
    };
  }
};
...
const getTranscriptById = async (transcriptId: string) => {
  ...
  try {
    ...
    const transcriptResult = await pool.query(
      `SELECT t.*, tm.messages FROM transcripts t LEFT JOIN transcript_messages tm ON t.id = tm.transcript_id WHERE t.id = $1`,
      [transcriptId],
    );
...
const getScenariosBySkill = async (skillId: string) => {
  console.log(`🔍 Getting scenarios for skill ID: ${skillId}`);
  try {
    // const client = await pool.connect();
    const result = await pool.query(
      "SELECT * FROM scenarios WHERE skill_id = $1 ORDER BY name",
      [skillId],
    );
...
const linkTranscriptToStreamingSession = async (
  transcriptId: string,
  streamingSessionId: string,
) => {
  console.log(
    `🔍 Linking transcript ${transcriptId} to streaming session ${streamingSessionId}`,
  );
  // const client = await pool.connect();
  try {
    console.log("🔍 Getting database client");
    ...
    await pool.query(
      `UPDATE streaming_sessions 
         SET transcript_id = $1
         WHERE id = $2`,
      [transcriptId, streamingSessionId],
    );
...
avatarSimulator.post("/custom-scenarios", async (req: any, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
    ...
    mapCustomScenarioToSkills(
      ...
      console.log(`Skill mapping completed for scenario ${scenario.id}:`, 
...
avatarSimulator.get("/custom-scenarios", async (req: any, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: "Authentication required" });
...
avatarSimulator.get("/custom-scenarios/:id", async (req: any, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: "Authentication required" });
...
avatarSimulator.delete("/custom-scenarios/:id", async (req: any, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: "Authentication required" });
...
avatarSimulator.get("/custom-scenarios/:id/skills", async (req: any, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: "Authentication required" });
...
avatarSimulator.post("/custom-scenarios/suggest-roles", async (req: any, res) => {
  try {
...
avatarSimulator.post("/custom-scenarios/:id/map-skills", async (req: any, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: "Authentication required" });
    ...
    const mappingResult = await mapCustomScenarioToSkills(
      scenarioId,
      scenario.title,
...
avatarSimulator.post("/custom-scenarios/analyze", async (req: any, res) => {
  try {
    const { scenarioDescription } = req.body;
    if (!scenarioDescription || typeof scenarioDescription !== "string") {
      return res.status(400).json({ error: "scenarioDescription is required" });
...
avatarSimulator.get("/presentation/feedback/:sessionId", async (req: any, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: "Authentication required" });
...
avatarSimulator.get("/presentation/scenarios/:id", async (req: any, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: "Authentication required" });
    ...
    const [presentation] = await db
      ...
      .where(and(
        eq(presentationScenarios.id, presentationId),
        eq(presentationScenarios.userId, userId)
      ));
...
avatarSimulator.get("/presentation/file/:id", async (req: any, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: "Authentication required" });
...
avatarSimulator.get("/presentation/view/:id", async (req: any, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: "Authentication required" });
...
avatarSimulator.post("/presentation/upload", upload.single("file"), async (req: any, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: "Authentication required" });
      ...
      try {
        ...
        if (docAnalysis) {
          await db
            ...
            .set({ documentAnalysis: docAnalysis as any })
...
avatarSimulator.get("/presentation/:id", async (req: any, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: "Authentication required" });
...
avatarSimulator.get("/presentation/:id/context", async (req: any, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: "Authentication required" });
    ...
    const context = generatePresentationContext(
      {
        fileName: presentation.fileName,
        totalSlides: presentation.totalSlides,
        slides: presentation.slidesData as any,
        extractedText: presentation.extractedText,
        fileType: (presentation.fileType as "pptx" | "pdf") || "pdf",
      },
      presentation.topic,
...
avatarSimulator.post("/presentation/:id/reparse", async (req: any, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: "Authentication required" });
...
avatarSimulator.post("/presentation/:id/feedback", async (req: any, res) => {
  try {
    const userId = req.user?.id;
    ...
    const [feedback, skillAssessment, documentAnalysis] = await Promise.all([
      ...
      generatePresentationSkillAssessment(
        transcript || "No transcript available",
        presentationData,
        presentation.topic
      ),
    ...
    const [savedFeedback] = await db
      ...
      .values({
        ...
        improvements: feedback.improvements,
        summary: feedback.summary,
        skillAssessment: skillAssessment as any,
...
avatarSimulator.get("/presentations", async (req: any, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: "Authentication required" });
...
avatarSimulator.get("/presentation-results", async (req: any, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: "Authentication required" });
...
avatarSimulator.get("/presentation-feedback/:sessionUid", async (req: any, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: "Authentication required" });
    ...
    const [session] = await db
      ...
      .leftJoin(presentationFeedbackTable, eq(presentationFeedbackTable.presentationSessionId, presentationSessions.id))
      .where(and(
        eq(presentationSessions.sessionUid, sessionUid),
        eq(presentationSessions.userId, userId)
      ))
      .limit(1);
...
avatarSimulator.post("/admin/full-data-migration", async (req, res) => {
  try {
    ...
    for (const { table, pk, isSerial } of migrationOrder) {
      try {
        const checkTableResult = await devClient.query(
