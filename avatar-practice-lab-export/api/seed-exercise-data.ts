  ...
  const caseTemplateData = [
    {
      ...
      caseType: "business_diagnosis" as const,
...
- Some customers report intermittent timeouts`,
    ...
    },
    {
      ...
      caseType: "execution_planning" as const,
      difficulty: "medium" as const,
...
class SlidingWindowRateLimiter:
    ...
    def get_retry_after(self, client_id: str) -> float:
      ...
      expectedSignals: [
        { signal: "Explains sliding window algorithm correctly", importance: "critical" as const },
        { signal: "Identifies thread safety with Lock", importance: "critical" as const },
        { signal: "Understands time complexity (O(n) for cleanup)", importance: "important" as const },
        { signal: "Discusses memory implications of storing timestamps", importance: "important" as const },
        { signal: "Suggests improvements like using sorted containers", importance: "nice_to_have" as const }
      ],
      commonFailureModes: [
...
const DataDashboard: React.FC = () => {
  ...
  return (
    <div>
      ...
      expectedSignals: [
        { signal: "Identifies missing WebSocket cleanup in useEffect", importance: "critical" as const },
        { signal: "Identifies missing interval cleanup", importance: "critical" as const },
        { signal: "Notes unbounded data growth issue", importance: "important" as const },
        { signal: "Explains React useEffect cleanup pattern", importance: "important" as const },
        { signal: "Provides correct fix with return cleanup function", importance: "critical" as const }
      ],
      commonFailureModes: [
...
class UserApiClient {
  ...
  async getUsers(userIds: string[]): Promise<User[]> {
    ...
    return users;
      ...
      expectedSignals: [
        { signal: "Adds cache data structure (Map with timestamps)", importance: "critical" as const },
        { signal: "Implements TTL expiration logic", importance: "critical" as const },
        { signal: "Handles cache invalidation", importance: "important" as const },
        { signal: "Considers cache for getUsers batch method", importance: "important" as const },
        { signal: "Discusses cache key strategy", importance: "nice_to_have" as const }
      ],
      commonFailureModes: [
...
def process_sales_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return monthly_summary`,
      ...
      expectedSignals: [
        { signal: "Explains each transformation step clearly", importance: "critical" as const },
        { signal: "Identifies potential issue with fillna(mean) before outlier removal", importance: "important" as const },
        { signal: "Notes date parsing might fail silently", importance: "important" as const },
        { signal: "Understands groupby aggregation", importance: "critical" as const },
        { signal: "Suggests improvements for production readiness", importance: "nice_to_have" as const }
      ],
      commonFailureModes: [
  ...
  const rubricData = [
    {
      ...
      exerciseType: "case_study" as const,
      dimensions: [
        {
    ...
    },
    {
      ...
      exerciseType: "coding_lab" as const,
      dimensions: [
        {
