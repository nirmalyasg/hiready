export const TECHNICAL_CATEGORIES = ["technical_interview", "coding_assessment", "system_design", "coding"];
export function normalizeRoundCategory(category) {
    if (TECHNICAL_CATEGORIES.includes(category)) {
        return "technical_interview";
    }
    return category;
}
export const ROUND_TAXONOMY = {
    hr_screening: {
...
export const BLUEPRINT_ROUND_MAPPING = {
...
export const ARCHETYPE_ROUND_DEFAULTS = {
...
export function generatePracticeOptions(jobTargetId, companyName, companyId, roleTitle, archetype, tier, blueprintRounds, blueprintNotes, focusAreas = [], leadershipPrinciples = null, blueprintFocusAreas = null) {
    const context = {
        jobTargetId,
        companyName,
        companyId,
    ...
    roundCategories = roundCategories.filter(cat => {
        if (cat === "coding_assessment" && !hasBlueprintCoding)
            return false;
        if (cat === "case_study" && !hasBlueprintCaseStudy)
            return false;
        return true;
    });
    ...
    const options = roundCategories.map((category, index) => {
...
function getFocusHint(category, archetype, blueprintNotes, leadershipPrinciples) {
