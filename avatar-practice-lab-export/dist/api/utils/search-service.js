...
export async function searchTopic(topic, options) {
...
export function formatResearchForPrompt(research, conversationStyle) {
...
export function getResearchQualityScore(research) {
    let score = 0;
    const reasons = [];
    ...
    if (highQualitySources >= 2)
        reasons.push(`${highQualitySources} high-quality sources`);
    else if (research.sources.length > 0)
        reasons.push(`${research.sources.length} sources`);
...
export function shouldResearchTopic(topic, category) {
...
export async function generateTopicInsights(topic) {
    const client = getTavilyClient();
    if (!client) {
    ...
    try {
        ...
        const sources = (response.results || []).slice(0, 5).map((r) => ({
            title: r.title || "",
            url: r.url || "",
        }));
        ...
        if (perspectives.length < 2) {
            perspectives.push({ viewpoint: "Key Consideration", explanation: keyFacts[1] || response.answer?.substring(0, 200) || "Multiple factors influence this topic." }, { viewpoint: "Broader Context", explanation: keyFacts[2] || "This topic connects to wider trends and developments." });
        }
