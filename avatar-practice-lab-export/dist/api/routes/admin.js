...
import { authUsers, users, userRoles, userLoginEvents, apiUsageEvents, adminSettings, userMediaPreferences, roleplaySession, skills, scenarios, sessionJourneyEvents, budgetGuards, budgetAlerts } from "../../shared/schema.js";
...
adminRouter.get("/analytics/summary", requireAdmin, async (req, res) => {
    try {
        ...
        const [totalUsersResult, activeUsersLast30Days, activeUsersLast7Days, totalSessionsResult, sessionsLast30Days, avgSessionDuration, totalLoginEvents] = await Promise.all([
            db.select({ count: count() }).from(authUsers),
            db.select({ count: sql `COUNT(DISTINCT ${roleplaySession.userId})` })
            ...
            db.select({ count: count() })
                ...
                .where(and(eq(userLoginEvents.eventType, "login"), gte(userLoginEvents.occurredAt, thirtyDaysAgo)))
        ]);
        res.json({
            success: true,
            data: {
...
adminRouter.get("/analytics/session-quality", requireAdmin, async (req, res) => {
    try {
      ...
      LIMIT 100
    `);
        const sessions = sessionsWithSQI.rows;
