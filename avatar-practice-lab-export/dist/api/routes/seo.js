...
import { seoPages, seoPageSections, seoInternalLinks, seoAnalyticsEvents, roleArchetypes, companies } from "../../shared/schema.js";
import { eq, desc, and, sql } from "drizzle-orm";
import { generatePillarPage, generateRolePrepPage, generateCompanyPrepPage, generateCompanyRolePage, generateSkillPracticePage, saveSeoPage } from "../lib/seo-content-generator.js";
const router = Router();
router.get("/pages", async (req, res) => {
    try {
        ...
        if (status && typeof status === "string") {
            conditions.push(eq(seoPages.status, status));
        }
        if (pageType && typeof pageType === "string") {
            conditions.push(eq(seoPages.pageType, pageType));
        }
...
router.post("/generate/pillar/:type", async (req, res) => {
    try {
...
router.post("/generate/role/:roleArchetypeId", async (req, res) => {
    try {
...
router.post("/generate/company/:companyId", async (req, res) => {
    try {
...
router.post("/generate/company-role", async (req, res) => {
    try {
...
router.post("/generate/skill/:skillType", async (req, res) => {
    try {
...
router.post("/generate/batch", async (req, res) => {
    try {
        ...
        const results = [];
        if (pageType === "pillar") {
            for (const type of ["mock-interview", "interview-preparation"]) {
                try {
...
router.patch("/pages/:id/unpublish", async (req, res) => {
    try {
        const { id } = req.params;
        const [updated] = await db.update(seoPages)
            .set({
...
router.get("/analytics/summary", async (req, res) => {
    try {
        const [totalPages] = await db.select({ count: sql `count(*)` }).from(seoPages);
        const [publishedPages] = await db.select({ count: sql `count(*)` })
            .from(seoPages)
            .where(eq(seoPages.status, "published"));
        const [totalViews] = await db.select({ sum: sql `coalesce(sum(view_count), 0)` })
            .from(seoPages);
        const [totalPracticeStarts] = await db.select({ sum: sql `coalesce(sum(practice_starts), 0)` })
            .from(seoPages);
...
router.get("/data/companies", async (req, res) => {
    try {
        ...
        const conditions = tier ? eq(companies.tier, tier) : undefined;
