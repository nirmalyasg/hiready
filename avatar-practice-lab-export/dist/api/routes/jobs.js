...
import { jobTargets, interviewConfigs, interviewSessions, exerciseSessions, interviewAnalysis, exerciseAnalysis, userSkillPatterns, roleKits, } from "../../shared/schema.js";
...
import { ROUND_TAXONOMY } from "../../shared/practice-context.js";
import { execSync } from "child_process";
import { resolveAndSaveJobArchetypes, listAllRoleArchetypes, listAllCompanyArchetypes, getRoleInterviewStructure, getEnrichedInterviewPlan } from "../lib/archetype-resolver.js";
...
async function findDuplicateJob(userId, options) {
    ...
    if (jobUrl) {
        ...
        const [existingByUrl] = await db
            ...
            .from(jobTargets)
            .where(and(eq(jobTargets.userId, userId), sql `REPLACE(SPLIT_PART(${jobTargets.jobUrl}, '?', 1), '/', '') = REPLACE(${normalizedUrl}, '/', '')`))
            .limit(1);
        if (existingByUrl)
    ...
    if (roleTitle && companyName) {
        const [existingByDetails] = await db
            .select({ id: jobTargets.id, roleTitle: jobTargets.roleTitle, companyName: jobTargets.companyName })
            .from(jobTargets)
            .where(and(eq(jobTargets.userId, userId), sql `LOWER(${jobTargets.roleTitle}) = LOWER(${roleTitle})`, sql `LOWER(${jobTargets.companyName}) = LOWER(${companyName})`))
            .limit(1);
    ...
    try {
        ...
        if (jdText && jdText.length > 50) {
            try {
                ...
                parsedJd = JSON.parse(response.choices[0].message.content || "{}");
            }
            catch (e) {
...
jobsRouter.post("/import-linkedin", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const { url } = req.body;
...
jobsRouter.get("/job-targets", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const { status, limit = "50" } = req.query;
        let conditions = [eq(jobTargets.userId, userId)];
        if (status && typeof status === "string" && status !== "all") {
            conditions.push(eq(jobTargets.status, status));
        }
        else {
...
jobsRouter.get("/job-targets/:id", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
...
jobsRouter.post("/job-targets", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        if (companyName || roleTitle) {
            try {
                archetypeInfo = await resolveAndSaveJobArchetypes(newJob.id, companyName || "", roleTitle, jdText || undefined);
            }
...
jobsRouter.put("/job-targets/:id", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const jobId = req.params.id;
...
jobsRouter.patch("/job-targets/:id/status", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const jobId = req.params.id;
...
jobsRouter.patch("/job-targets/:id/role-kit", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const jobId = req.params.id;
...
jobsRouter.delete("/job-targets/:id", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const jobId = req.params.id;
        const [existing] = await db
...
function detectPortal(url) {
    ...
    if (hostname.includes('linkedin.com'))
        return 'linkedin';
    if (hostname.includes('indeed.com') || hostname.includes('indeed.co'))
        return 'indeed';
    if (hostname.includes('glassdoor.com') || hostname.includes('glassdoor.co'))
        return 'glassdoor';
    if (hostname.includes('naukri.com'))
        return 'naukri';
    if (hostname.includes('monster.com') || hostname.includes('monster.co'))
        return 'monster';
...
jobsRouter.post("/job-targets/parse-url", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const { url } = req.body;
        if (!url) {
        ...
        try {
            ...
            const isLoginBlocked = currentUrl.includes('/login') ||
            ...
            await new Promise(r => setTimeout(r, 2000));
            const extractWithSelectors = async (selectors) => {
...
jobsRouter.post("/job-targets/:id/parse-jd", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const jobId = req.params.id;
        const [job] = await db
...
jobsRouter.post("/job-targets/parse-paste", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
...
jobsRouter.get("/job-targets/:id/practice-suggestions", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const jobId = req.params.id;
        const [job] = await db
        ...
        const preliminaryCompanyData = await getCompanyAwarePracticeOptions(job.companyName, job.roleTitle, null);
        if (!parsed) {
            return res.json({
                success: true,
                suggestions: [{
        ...
        const { options, companyData } = await getCompanyAwarePracticeOptions(job.companyName, job.roleTitle, {
...
jobsRouter.get("/job-targets/:id/practice-options", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const jobId = req.params.id;
        const [job] = await db
        ...
        const parsed = job.jdParsed;
        const blueprintData = await findCompanyBlueprint(job.companyName, job.roleTitle, parsed?.experienceLevel);
        const companyNotes = blueprintData?.blueprint?.notes || null;
        const interviewPlan = await getEnrichedInterviewPlan(job.roleArchetypeId || null, job.roleFamily || null, job.companyArchetype || null, job.archetypeConfidence, parsed?.experienceLevel || null, companyNotes, job.companyName || null);
        ...
        const options = interviewPlan.phases
            ...
            const taxonomy = ROUND_TAXONOMY[normalizedCategory];
            return {
...
jobsRouter.get("/job-targets/:id/practice-context/:roundCategory", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const jobId = req.params.id;
        const roundCategory = req.params.roundCategory;
        const [job] = await db
            .select()
            .from(jobTargets)
            .where(and(eq(jobTargets.id, jobId), eq(jobTargets.userId, userId)));
        ...
        const companyContext = {
            ...
            focusAreas: parsed?.focusAreas || [],
            leadershipPrinciples: blueprintData?.blueprint?.skillFocus?.filter(s => s.includes("ownership") || s.includes("leadership") || s.includes("customer")) || null,
        ...
        const matchingRound = blueprintData?.blueprint?.interviewRounds?.find(r => {
            ...
            if (roundCategory === "aptitude_assessment" && roundLower.includes("aptitude"))
                return true;
            if (roundCategory === "hr_screening" && (roundLower.includes("phone") || roundLower.includes("hr")))
                return true;
            if (roundCategory === "hiring_manager" && roundLower.includes("hiring"))
                return true;
            if (roundCategory === "technical_interview" && roundLower.includes("technical"))
                return true;
            if (roundCategory === "coding_assessment" && (roundLower.includes("coding") || roundLower.includes("dsa")))
                return true;
            if (roundCategory === "system_design" && roundLower.includes("system"))
                return true;
            if (roundCategory === "case_study" && roundLower.includes("case"))
                return true;
            if (roundCategory === "behavioral" && roundLower.includes("behavioral"))
                return true;
            if (roundCategory === "culture_values" && (roundLower.includes("culture") || roundLower.includes("values")))
                return true;
            if (roundCategory === "bar_raiser" && roundLower.includes("bar_raiser"))
                return true;
            if (roundCategory === "group_discussion" && roundLower.includes("group"))
                return true;
            return false;
        });
        res.json({
...
function generatePromptHints(roundCategory, context, roundDetails) {
    ...
    let companySpecificGuidance = null;
...
jobsRouter.post("/job-targets/:id/compute-readiness", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const jobId = req.params.id;
        const [job] = await db
        ...
        const interviewScores = interviewSessionsForJob
            ...
            .map(s => {
            const scores = s.dimensionScores.map(d => d.score);
            return scores.length > 0 ? scores.reduce((a, b) => a + b, 0) / scores.length : null;
        })
            .filter((s) => s !== null);
        const allScores = [
            ...interviewScores,
            ...exerciseSessionsForJob.filter(s => s.score !== null).map(s => s.score),
        ];
...
jobsRouter.get("/job-targets/:id/readiness", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const jobId = req.params.id;
        ...
        for (const session of interviewSessionsWithAnalysis) {
            if (session.dimensionScores && Array.isArray(session.dimensionScores)) {
                for (const dim of session.dimensionScores) {
                    ...
                    if (dim.evidence)
                        dimensionAggregates[dim.dimension].evidence.push(...dim.evidence);
                    if (dim.improvement)
                        dimensionAggregates[dim.dimension].improvements.push(dim.improvement);
                }
            }
        }
        ...
        const focusAreas = job.jdParsed?.focusAreas || [];
        ...
        if (improvingDimensions > decliningDimensions + 1)
            overallTrend = "improving";
        else if (decliningDimensions > improvingDimensions + 1)
...
jobsRouter.get("/skill-patterns", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
...
jobsRouter.get("/ai-insights", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const patterns = await db
            .select()
...
jobsRouter.post("/skill-patterns/update", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        for (const dim of dimensions) {
            ...
            const [existing] = await db
                .select()
                .from(userSkillPatterns)
                .where(and(eq(userSkillPatterns.userId, userId), eq(userSkillPatterns.dimension, name)));
            if (existing) {
                ...
                // Determine trend
                let trend = "stagnant";
                if (prevAvg && score > prevAvg + 5)
                    trend = "improving";
                else if (prevAvg && score < prevAvg - 5)
                    trend = "declining";
                const [updated] = await db
...
jobsRouter.get("/archetypes/roles", requireAuth, async (req, res) => {
    try {
...
jobsRouter.get("/archetypes/companies", requireAuth, async (req, res) => {
    try {
...
jobsRouter.get("/archetypes/structure/:roleArchetypeId/:seniority", requireAuth, async (req, res) => {
    try {
        ...
        const structure = await getRoleInterviewStructure(roleArchetypeId, seniority);
        if (!structure) {
...
jobsRouter.put("/job-targets/:id/archetype", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const jobId = req.params.id;
        ...
        if (autoDetect) {
            const archetypeInfo = await resolveAndSaveJobArchetypes(jobId, existing.companyName || "", existing.roleTitle, existing.jdText || undefined);
            const [updated] = await db
                .select()
                .from(jobTargets)
                .where(eq(jobTargets.id, jobId));
...
jobsRouter.post("/job-targets/:id/resolve-archetypes", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const jobId = req.params.id;
        const [existing] = await db
