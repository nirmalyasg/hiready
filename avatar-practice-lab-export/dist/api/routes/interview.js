...
import { roleKits, roleArchetypes, interviewRubrics, userDocuments, userProfileExtracted, interviewConfigs, interviewPlans, interviewSessions, interviewAnalysis, jobTargets, userSkillPatterns, companies, jobPracticeLinks, codingExercises, roleTaskBlueprints, roleInterviewStructureDefaults, } from "../../shared/schema.js";
import { extractSkillsFromText, combineSkillProfiles, matchExercisesToProfile, generateCompositeProblem, } from "../lib/capability-vectorizer.js";
...
import { loadQuestionPatterns, getCompanyBlueprint, generateEnhancedPlan, classifyAnswer, selectProbe, updateUserSkillMemory, getUserSkillTrends, calculateReadinessScore, getJobReadinessSummary, generateSevenDayPlan, generateAIPracticePlan, buildDynamicRubric, getQuestionsWithCoverage, } from "../lib/interview-intelligence.js";
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
export const interviewRouter = Router();
// Normalize exercise content: ensure both singular and array fields are populated
function normalizePlanExercises(planJson, interviewMode) {
...
function mapPhasesToFrontendFormat(planJson) {
    if (!planJson || !planJson.phases)
        return planJson;
    return {
        ...planJson,
        phases: planJson.phases.map((phase) => ({
            name: phase.name,
...
function inferPhaseType(id, name) {
...
const INTERVIEW_MODE_TO_TASK_TYPES = {
...
async function loadBlueprintsForMode(roleArchetypeId, interviewMode, seniority) {
    ...
    const seniorityMap = { entry: "l1_l2", mid: "l3_l4", senior: "l5_plus" };
    const mappedSeniority = seniorityMap[seniority] || "l3_l4";
    const [structureDefaults] = await db
        .select()
        .from(roleInterviewStructureDefaults)
        .where(and(eq(roleInterviewStructureDefaults.roleArchetypeId, roleArchetypeId), eq(roleInterviewStructureDefaults.seniority, mappedSeniority)))
        .limit(1);
...
// Role Kit Endpoints
// ===================================================================
interviewRouter.get("/role-kits", async (req, res) => {
    try {
        ...
        if (search && typeof search === "string") {
            ...
            filtered = filtered.filter(k => k.name.toLowerCase().includes(searchLower) ||
...
interviewRouter.get("/role-kits/:id", async (req, res) => {
    try {
...
interviewRouter.get("/role-archetypes", async (req, res) => {
    try {
...
import { getRolePracticeOptions } from "../lib/archetype-resolver.js";
interviewRouter.get("/role-kits/:id/practice-options", async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        if (isNaN(id)) {
...
// ===================================================================
interviewRouter.post("/documents/upload", requireAuth, upload.single("file"), async (req, res) => {
    try {
        if (!req.file) {
...
interviewRouter.get("/documents", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
...
interviewRouter.get("/documents/:id", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const id = parseInt(req.params.id);
        const [doc] = await db
...
interviewRouter.post("/documents/:id/parse", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const id = parseInt(req.params.id);
        const [doc] = await db
...
interviewRouter.delete("/documents/:id", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const id = parseInt(req.params.id);
        const [doc] = await db
...
// ===================================================================
interviewRouter.post("/config", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const { roleKitId, roleArchetypeId, interviewMode: requestedMode, resumeDocId: providedResumeDocId, jdDocId, companyNotesDocId, interviewType, style, seniority, mode, jobTargetId, exerciseCount, includePuzzles, employerJobId, } = req.body;
...
interviewRouter.get("/config/:id", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const id = parseInt(req.params.id);
        const [config] = await db
...
// ===================================================================
interviewRouter.post("/config/:id/plan", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        let jdParsed = null;
        let roleKitData = null;
        let jobTargetData = null;
        let roleArchetypeData = null;
        let blueprintData = { taskBlueprints: [], structureDefaults: null };
        if (config.resumeDocId) {
            const [doc] = await db.select().from(userDocuments).where(eq(userDocuments.id, config.resumeDocId));
        ...
        if (config.roleArchetypeId) {
            ...
            if (config.interviewMode && INTERVIEW_MODE_TO_TASK_TYPES[config.interviewMode]) {
                blueprintData = await loadBlueprintsForMode(config.roleArchetypeId, config.interviewMode, config.seniority);
            }
        }
        let jobTargetForRichContext = null;
        ...
        const jdExtractForQuestions = jdParsed ? {
...
interviewRouter.get("/plan/:id", requireAuth, async (req, res) => {
    try {
        const id = parseInt(req.params.id);
...
// ===================================================================
interviewRouter.post("/session/start", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        if (config.interviewType === 'case_study') {
            ...
            const caseStudyPhase = planData.phases?.find((p) => p.name?.toLowerCase().includes('case') ||
...
interviewRouter.post("/session/:id/link-roleplay", requireAuth, async (req, res) => {
    try {
...
interviewRouter.post("/session/:id/end", requireAuth, async (req, res) => {
    try {
        const sessionId = parseInt(req.params.id);
        const [updated] = await db
            .update(interviewSessions)
...
interviewRouter.post("/session/:id/code-review", requireAuth, async (req, res) => {
    try {
...
interviewRouter.get("/session/:id", requireAuth, async (req, res) => {
    try {
        const sessionId = parseInt(req.params.id);
        ...
        let mappedPlan = mapPhasesToFrontendFormat(plan?.planJson);
        ...
        if (isCaseStudyMode && mappedPlan && !mappedPlan.caseStudy) {
            const caseStudyPhase = mappedPlan.phases?.find((p) => p.name?.toLowerCase().includes('case') ||
                p.practiceMode === 'case_study' ||
                p.category === 'case_study');
...
// ===================================================================
interviewRouter.post("/session/:id/analyze", requireAuth, async (req, res) => {
    try {
        const sessionId = parseInt(req.params.id);
        ...
        const extractSkillsFromParsed = (parsed) => {
            ...
            return [...new Set(allSkills.filter((s) => typeof s === 'string' && s.trim().length > 0))];
        };
        ...
        if (jdSkills.length === 0 && session.jobTargetId) {
            ...
            if (jobTarget) {
                ...
                if (jdSkills.length === 0) {
                    const topLevelDimensions = jobTarget.analysisDimensions || [];
                    const topLevelTopics = jobTarget.interviewTopics || [];
                    if (topLevelDimensions.length > 0 || topLevelTopics.length > 0) {
                        jdSkills = [...topLevelDimensions, ...topLevelTopics].filter((s) => typeof s === 'string' && s.trim().length > 0);
                    }
                }
            }
        }
        ...
        for (const dimScore of dimensionScores) {
            if (!dimScore.dimension || typeof dimScore.score !== "number")
                continue;
            const [existingPattern] = await db
                .select()
                .from(userSkillPatterns)
                .where(and(eq(userSkillPatterns.userId, userId), eq(userSkillPatterns.dimension, dimScore.dimension)));
            if (existingPattern) {
                ...
                let trend = "stagnant";
                if (dimScore.score > oldAvg + 0.3)
                    trend = "improving";
                else if (dimScore.score < oldAvg - 0.3)
                    trend = "declining";
                await db
...
interviewRouter.get("/analysis/:sessionId", requireAuth, async (req, res) => {
    try {
...
// ===================================================================
interviewRouter.get("/knowledge-base/:configId", requireAuth, async (req, res) => {
    try {
...
function buildInterviewKnowledgeBase(data) {
    ...
    if (roleKit) {
        ...
        if (roleKit.skillsFocus) {
            kb += `Key Skills to Assess: ${roleKit.skillsFocus.join(", ")}\n`;
        }
        kb += "\n";
    }
    if (resumeParsed) {
        kb += "=== CANDIDATE PROFILE ===\n";
        if (resumeParsed.headline)
            kb += `Headline: ${resumeParsed.headline}\n`;
        if (resumeParsed.skills)
            kb += `Skills: ${resumeParsed.skills.join(", ")}\n`;
        if (resumeParsed.workHistory) {
            kb += "Work History:\n";
            resumeParsed.workHistory.forEach((w) => {
                kb += `- ${w.role} at ${w.company} (${w.duration})\n`;
            });
        }
        if (resumeParsed.riskFlags && resumeParsed.riskFlags.length > 0) {
            kb += "Areas to Probe:\n";
            resumeParsed.riskFlags.forEach((f) => {
    ...
    if (jdParsed) {
        kb += "=== JOB REQUIREMENTS ===\n";
        if (jdParsed.title)
            kb += `Title: ${jdParsed.title}\n`;
        if (jdParsed.company)
            kb += `Company: ${jdParsed.company}\n`;
        if (jdParsed.requiredSkills)
            kb += `Required Skills: ${jdParsed.requiredSkills.join(", ")}\n`;
        if (jdParsed.preferredSkills)
            kb += `Nice to Have: ${jdParsed.preferredSkills.join(", ")}\n`;
        if (jdParsed.responsibilities) {
            kb += "Responsibilities:\n";
            jdParsed.responsibilities.forEach((r) => {
                kb += `- ${r}\n`;
            });
        }
        kb += "\n";
    ...
    if (plan) {
        ...
        if (plan.phases) {
            plan.phases.forEach((phase) => {
                ...
                if (phase.objectives) {
                    phase.objectives.forEach((o) => kb += `  - ${o}\n`);
                }
            });
        }
        if (plan.focusAreas) {
        ...
        if (plan.keyQuestions && Array.isArray(plan.keyQuestions)) {
            ...
            plan.keyQuestions.slice(0, 5).forEach((q) => {
                kb += `- ${q}\n`;
            });
        }
        kb += "\n";
...
function buildComplexityCalibrationSection(seniority) {
...
// =====================
interviewRouter.get("/intelligence/patterns", requireAuth, async (req, res) => {
    try {
        const { roleCategory = "general", interviewType = "hiring_manager", types } = req.query;
        const patternTypes = types ? types.split(",") : ["resume_claim", "behavioral", "jd_requirement"];
        const patterns = await loadQuestionPatterns(roleCategory, interviewType, patternTypes);
...
interviewRouter.get("/intelligence/companies", async (req, res) => {
    try {
        const { tier, archetype, search } = req.query;
        const conditions = [];
        if (tier) {
            conditions.push(eq(companies.tier, tier));
        }
        if (archetype) {
            conditions.push(eq(companies.archetype, archetype));
        }
        let results;
...
interviewRouter.get("/intelligence/company/:name/blueprint", async (req, res) => {
    try {
        const { name } = req.params;
        const { roleCategory = "swe" } = req.query;
        const result = await getCompanyBlueprint(name, roleCategory);
        if (!result) {
...
interviewRouter.post("/intelligence/plan/generate", requireAuth, async (req, res) => {
    try {
        ...
        if (context.companyName && !context.blueprintFocus) {
            ...
            if (blueprintData?.blueprint) {
                const interviewRounds = blueprintData.blueprint.interviewRounds || [];
                const focusAreas = interviewRounds.flatMap(r => r.focus || []);
                const focusLower = focusAreas.map(f => f.toLowerCase());
                const hasCodingRounds = focusLower.some(f => ["coding", "dsa", "algorithms", "machine-coding", "leetcode"].includes(f));
                const hasCaseStudyRounds = focusLower.some(f => ["case", "case-study", "estimation", "market-sizing", "business-case"].includes(f));
                const hasSystemDesign = focusLower.some(f => ["system-design", "system_design", "architecture", "scale"].includes(f));
...
interviewRouter.post("/intelligence/answer/classify", requireAuth, async (req, res) => {
    try {
...
interviewRouter.post("/intelligence/probe/select", requireAuth, async (req, res) => {
    try {
...
interviewRouter.get("/intelligence/skill-memory", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const { roleCategory } = req.query;
        const trends = await getUserSkillTrends(userId, roleCategory);
...
interviewRouter.post("/intelligence/skill-memory/update", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
...
interviewRouter.post("/practice-link", requireAuth, async (req, res) => {
    try {
...
interviewRouter.get("/practice-links/:jobTargetId", requireAuth, async (req, res) => {
    try {
...
// =====================
interviewRouter.get("/readiness/:jobTargetId", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const { jobTargetId } = req.params;
        const [job] = await db
        ...
        const jobForReadiness = {
            id: job.id,
            roleTitle: job.roleTitle || "General",
            company: job.company || undefined,
            jdText: job.jdRaw || undefined,
            mustHaveSkills: job.jdParsed?.mustHave || [],
            niceToHaveSkills: job.jdParsed?.niceToHave || [],
            seniority: job.jdParsed?.senioritySignal || undefined,
        };
...
interviewRouter.get("/readiness-summary", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const jobs = await db
            .select()
        ...
        const jobsForReadiness = jobs.map(job => ({
            id: job.id,
            roleTitle: job.roleTitle || "General",
            company: job.company || undefined,
            jdText: job.jdRaw || undefined,
...
function detectRoleCategoryFromTitle(roleTitle) {
    const lower = roleTitle.toLowerCase();
    if (lower.includes("engineer") || lower.includes("developer") || lower.includes("swe"))
        return "swe";
...
// =====================
interviewRouter.post("/coach/plan", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const { jobTargetId, dailyTimeAvailable = 45, useAI = true, interviewDate, } = req.body;
        let jobForReadiness;
        ...
        const readiness = await calculateReadinessScore(userId, jobForReadiness || { id: 0, roleTitle: "General" }, roleCategory);
...
interviewRouter.get("/coach/plan/:jobTargetId", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const { jobTargetId } = req.params;
        ...
        const plan = await generateSevenDayPlan({
            ...
            jobTarget: jobForReadiness,
            roleCategory,
            dailyTimeAvailable: parseInt(dailyTimeAvailable) || 45,
        });
        res.json({
            success: true,
            plan,
...
interviewRouter.get("/coach/today", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
...
// ===================================================================
interviewRouter.post("/match-exercise", requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        const matchedExercises = matchExercisesToProfile(availableExercises.map(e => ({
            id: e.id,
            name: e.name,
        ...
        if (profile.combinedRequirements.length > 0) {
            try {
                ...
                const generated = await generateCompositeProblem(profile, availableExercises.map(e => ({ name: e.name, activityType: e.activityType })), openai);
                return res.json({
                    success: true,
                    exercise: {
                        id: null,
...
// =====================
import { calculateConsolidatedHireadyIndex, saveHireadyIndex, getPublicHireadyIndex } from "../lib/hiready-index.js";
import { hireadyRoleIndex } from "../../shared/schema.js";
interviewRouter.get("/hiready-index", requireAuth, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
...
interviewRouter.post("/hiready-index/save", requireAuth, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ success: false, error: "User not authenticated" });
...
interviewRouter.post("/hiready-index/:id/share", requireAuth, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ success: false, error: "User not authenticated" });
        ...
        await db
            ...
            .where(and(eq(hireadyRoleIndex.id, id), eq(hireadyRoleIndex.userId, userId)));
        const [updated] = await db
            .select()
...
interviewRouter.get("/hiready-index/public/:shareToken", async (req, res) => {
    try {
...
interviewRouter.get("/hiready-index/user-summary", requireAuth, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ success: false, error: "User not authenticated" });
...
interviewRouter.post("/session/:sessionId/share", requireAuth, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ success: false, error: "User not authenticated" });
        ...
        const [session] = await db
            ...
            .where(and(eq(interviewSessions.id, sessionIdNum), eq(interviewConfigs.userId, userId)))
            .limit(1);
        if (!session) {
            return res.status(404).json({ success: false, error: "Session not found" });
        }
...
// Toggle share status
interviewRouter.patch("/session/:sessionId/share", requireAuth, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ success: false, error: "User not authenticated" });
...
interviewRouter.get("/results/public/:shareToken", async (req, res) => {
    try {
        const { shareToken } = req.params;
        const [analysis] = await db
            .select()
            .from(interviewAnalysis)
            .where(and(eq(interviewAnalysis.shareToken, shareToken), eq(interviewAnalysis.isPublic, true)))
            .limit(1);
        if (!analysis) {
            return res.status(404).json({
                success: false,
