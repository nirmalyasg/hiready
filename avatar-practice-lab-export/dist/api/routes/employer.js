...
import { employerCompanies, employerJobs, employerCandidates, authUsers, } from "../../shared/schema.js";
...
employerRouter.get("/my-company", requireEmployerAuth, async (req, res) => {
    try {
        const employerUser = req.employerUser;
...
employerRouter.post("/jobs", requireEmployerAuth, async (req, res) => {
    try {
        const employerUser = req.employerUser;
        if (!employerUser?.companyId) {
        ...
        if (jdText || title) {
            try {
                generatedInterviewPlan = await buildEmployerInterviewPlan(title, jdText || "", company?.name || undefined, "mid");
...
employerRouter.get("/jobs", requireEmployerAuth, async (req, res) => {
    try {
        const employerUser = req.employerUser;
        if (!employerUser?.companyId) {
            return res.status(403).json({ success: false, error: "No company associated with this account" });
...
employerRouter.get("/jobs/:jobId", requireEmployerAuth, async (req, res) => {
    try {
        const employerUser = req.employerUser;
...
employerRouter.put("/jobs/:jobId", requireEmployerAuth, async (req, res) => {
    try {
        const employerUser = req.employerUser;
        const { jobId } = req.params;
        const [job] = await db
...
employerRouter.delete("/jobs/:jobId", requireEmployerAuth, async (req, res) => {
    try {
        const employerUser = req.employerUser;
        const { jobId } = req.params;
        const [job] = await db
...
employerRouter.get("/jobs/:jobId/candidates", requireEmployerAuth, async (req, res) => {
    try {
        const employerUser = req.employerUser;
        const { jobId } = req.params;
        const [job] = await db
...
employerRouter.get("/jobs/:jobId/candidates/export", requireEmployerAuth, async (req, res) => {
    try {
        const employerUser = req.employerUser;
        const { jobId } = req.params;
        const [job] = await db
...
employerRouter.put("/candidates/:candidateId/status", requireEmployerAuth, async (req, res) => {
    try {
        const employerUser = req.employerUser;
...
employerRouter.get("/candidates/:candidateId/analysis", requireEmployerAuth, async (req, res) => {
    try {
        const employerUser = req.employerUser;
        const { candidateId } = req.params;
        const [candidate] = await db
...
employerRouter.get("/dashboard/stats", requireEmployerAuth, async (req, res) => {
    try {
        const employerUser = req.employerUser;
        if (!employerUser?.companyId) {
            return res.status(403).json({ success: false, error: "No company associated with this account" });
