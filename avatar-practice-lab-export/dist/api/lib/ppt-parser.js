...
async function parsePdfWithLib(buffer) {
    try {
...
export function isPptxFile(fileName) {
...
function isValidSlideText(text) {
    ...
    if (/^\{[A-F0-9-]+\}$/i.test(text))
        return false;
    if (/^rId\d+$/i.test(text))
        return false;
    if (/^[0-9]+$/.test(text) && text.length < 4)
        return false;
    if (/^[a-z]$/.test(text))
        return false;
    if (text.includes("schemas.openxmlformats") || text.includes("schemas.microsoft"))
        return false;
    if (/^(Freeform|TextBox|Group)\s+\d+$/i.test(text))
        return false;
    return true;
}
function cleanSlideTexts(texts) {
...
function extractTextFromXml(obj) {
    ...
    function traverse(node) {
        ...
        if (typeof node === "string") {
            ...
            if (trimmed && isValidSlideText(trimmed))
                texts.push(trimmed);
            return;
        }
...
function categorizeSlideTexts(texts) {
    if (texts.length === 0) {
...
export async function parsePptxBuffer(buffer, fileName) {
    ...
    for (let i = 0; i < slideFiles.length; i++) {
        ...
        try {
            ...
            if (!slideContent)
                continue;
            const parsed = await parseXml(slideContent);
            ...
            if (zip.files[notesFile]) {
                try {
                    ...
                    if (notesContent) {
                        const notesParsed = await parseXml(notesContent);
...
export async function parsePresentation(buffer, fileName) {
...
export function generatePresentationContext(presentation, topic, userContext) {
