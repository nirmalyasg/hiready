...
import { subscriptions, interviewSessions, interviewConfigs, employerCandidates, jobTargets, } from "../../shared/schema.js";
export async function checkEntitlements(userId, options) {
    const { jobTargetId, roleKitId, employerJobId } = options;
    if (employerJobId) {
        const [candidate] = await db
            .select()
            .from(employerCandidates)
            .where(and(eq(employerCandidates.userId, userId), eq(employerCandidates.jobId, employerJobId)))
            .limit(1);
        if (candidate) {
            return {
                hasAccess: true,
    ...
    if (targetRoleKitId) {
        const [rolePackSubscription] = await db
            .select()
            .from(subscriptions)
            .where(and(eq(subscriptions.userId, userId), eq(subscriptions.planType, "role_pack"), eq(subscriptions.roleKitId, targetRoleKitId), eq(subscriptions.status, "active")))
            .orderBy(desc(subscriptions.createdAt))
            .limit(1);
    ...
    const completedSessionsResult = await db
        ...
        .where(and(eq(interviewConfigs.userId, userId), eq(interviewConfigs.roleKitId, targetRoleKitId)));
...
export async function enforceEntitlements(userId, options) {
