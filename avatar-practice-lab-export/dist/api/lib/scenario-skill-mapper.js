...
function buildMappingPrompt(scenarioTitle, scenarioDescription, scenarioBlueprint, skillCatalog) {
...
export async function analyzeScenarioForSkills(customScenarioId, title, description, blueprint, pool) {
...
export async function saveScenarioSkillMappings(result, pool) {
    await pool.query("DELETE FROM custom_scenario_skill_mappings WHERE custom_scenario_id = $1", [result.customScenarioId]);
...
export async function mapCustomScenarioToSkills(customScenarioId, title, description, blueprint, pool) {
    const result = await analyzeScenarioForSkills(customScenarioId, title, description, blueprint, pool);
    await saveScenarioSkillMappings(result, pool);
    return result;
}
export async function getSkillMappingsForCustomScenario(customScenarioId, pool) {
