...
function categorizeSkill(skill) {
    ...
    if (languages.some(l => lower.includes(l)))
        return "language";
...
export function extractSkillsFromText(text, source) {
    const skillPatterns = [
...
export function combineSkillProfiles(jdSkills, resumeSkills, roleKitSkills) {
...
function determineSeniorityLevel(skills) {
    ...
    if (conceptSkills.length >= 5 && totalWeight > 20)
        return "lead";
    if (conceptSkills.length >= 3 && totalWeight > 15)
        return "senior";
    if (totalWeight > 8)
        return "mid";
    return "junior";
}
export async function generateCompositeProblem(profile, existingExercises, openai) {
...
export function matchExercisesToProfile(exercises, profile) {
    const results = [];
