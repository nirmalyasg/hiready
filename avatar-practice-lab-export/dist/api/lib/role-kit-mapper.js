...
export async function mapRoleTitleToRoleKit(roleTitle, roleFamily, jdText) {
    ...
    for (const kit of allRoleKits) {
        ...
        if (kitName.includes(normalizedTitle) || normalizedTitle.includes(kitName.split(" - ")[0])) {
            const alternatives = allRoleKits
                ...
                .slice(0, 3)
                .map(k => ({ roleKitId: k.id, roleKitName: k.name, confidence: "medium" }));
            return {
                roleKitId: kit.id,
    ...
    for (const [domain, patterns] of Object.entries(ROLE_DOMAIN_MAP)) {
        const matchCount = patterns.filter(p => normalizedTitle.includes(p) || normalizedJd.includes(p)).length;
    ...
    if (detectedDomain) {
        ...
        if (matchingKits.length > 0) {
            const entryLevel = matchingKits.find(k => k.name.toLowerCase().includes("entry") || k.name.toLowerCase().includes("associate"));
    ...
    const topAlternatives = allRoleKits
        ...
        .slice(0, 5)
        .map(k => ({ roleKitId: k.id, roleKitName: k.name, confidence: "low" }));
    if (defaultKit) {
        return {
