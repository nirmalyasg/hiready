...
import { roleArchetypes, roleInterviewStructureDefaults, companies, questionPatterns, companyRoleBlueprints, seoPages, seoPageSections } from "../../shared/schema.js";
...
async function generateNarrativeContent(prompt) {
    try {
...
export async function generatePillarPage(type) {
    ...
    if (type === "mock-interview") {
        ...
        sections.push({
            ...
            content: await generateNarrativeContent(`Explain the main categories of interview questions candidates face, using these pattern types: ${patternTypes.join(", ")}. For each category, describe what interviewers are really probing for beneath the surface question, and how the same question type reveals different signals across candidate responses.`),
        ...
        sections.push({
            ...
            content: await generateNarrativeContent(`Make the case for active practice over passive reading. Explain how repeated exposure changes response quality, confidence, and clarity under pressure. Show the difference between candidates who "know the answers" versus those who have practiced saying them out loud in real time.`),
            sourceTable: "editorial"
        });
        sections.push({
            headingType: "h2",
...
export async function generateRolePrepPage(roleArchetypeId) {
    ...
    const patterns = await db.select().from(questionPatterns)
        .where(eq(questionPatterns.roleCategory, roleCategory))
        .limit(15);
    ...
    if (structure?.phasesJson) {
        sections.push({
            ...
            content: await generateNarrativeContent(`Describe the interview structure commonly used for ${roleName} roles, based on this data: ${JSON.stringify(structure.phasesJson)}. Explain how expectations shift across rounds and what candidates often underestimate at each stage. Focus on preparation mistakes candidates make for each phase.`),
            sourceTable: "role_interview_structure_defaults",
    ...
    if (patterns.length > 0) {
        ...
        sections.push({
            ...
            content: await generateNarrativeContent(`Describe the recurring question patterns ${roleName} candidates face, using these examples: ${patternSamples.join("; ")}. Explain why interviewers rely on these patterns and what they reveal about candidates beyond the surface answer. Emphasize structure and reasoning, not memorization.`),
            sourceTable: "question_patterns",
    ...
    sections.push({
        ...
        content: await generateNarrativeContent(`Explain the most common ways ${roleName} candidates fail interviews, based on these failure modes: ${failureModes.join("; ")}. Describe how these mistakes typically appear in conversation and why candidates don't realize they're happening. Keep the tone diagnostic, not judgmental.`),
        sourceTable: "role_archetypes",
...
export async function generateCompanyPrepPage(companyId) {
    ...
    if (blueprints.length > 0) {
        ...
        sections.push({
            ...
            content: await generateNarrativeContent(`Explain which skills ${companyName} consistently prioritizes during interviews, based on: ${skillFocusAreas.join(", ")}. Describe how interviewers test these skills indirectly through questions, follow-ups, and problem framing. Avoid stating values — show how they surface in interviews.`),
    ...
    sections.push({
        ...
        content: await generateNarrativeContent(`Describe the types of interview questions ${companyName} is known for asking. Given their ${company.archetype} archetype in the ${company.sector} sector, explain the typical question styles, how interviewers probe for depth, and the subtle signals they're evaluating beyond the answer itself.`),
        sourceTable: "question_patterns"
    });
...
export async function generateCompanyRolePage(companyId, roleArchetypeId) {
    const [company] = await db.select().from(companies).where(eq(companies.id, companyId));
    const [archetype] = await db.select().from(roleArchetypes).where(eq(roleArchetypes.id, roleArchetypeId));
...
export async function generateSkillPracticePage(skillType) {
    const skillTypes = {
    ...
    if (!skill)
        return null;
    const patterns = await db.select().from(questionPatterns)
        .where(eq(questionPatterns.interviewType, skill.interviewType))
        .limit(15);
    const sections = [];
    sections.push({
        headingType: "h2",
        heading: "What This Interview Tests",
        content: await generateNarrativeContent(`Explain what a ${skill.name} interview truly evaluates beyond correctness. Describe how interviewers judge thinking process, communication clarity, and adaptability under questioning. Explain how this interview type differs in tone and expectations from others.`),
        sourceTable: "editorial"
    });
    if (patterns.length > 0) {
        sections.push({
            headingType: "h2",
            heading: "Common Question Patterns",
            content: await generateNarrativeContent(`Describe the most common question structures used in ${skill.name} interviews. Use examples like: ${patterns.slice(0, 5).map(p => p.template?.substring(0, 50)).join("; ")}. Explain the logic behind these patterns and what interviewers are trying to surface about the candidate.`),
            sourceTable: "question_patterns",
            sourceData: { patternCount: patterns.length }
        });
    }
    sections.push({
        ...
        content: await generateNarrativeContent(`Explain how interviewers evaluate answers during ${skill.name} interviews in real time. Describe what makes an answer feel confident and trustworthy versus risky or unclear. Highlight subtle signals interviewers react to, including pacing, framing, and handling follow-ups.`),
        sourceTable: "editorial"
    });
    sections.push({
        headingType: "h2",
...
export async function saveSeoPage(page, roleArchetypeId, companyId) {
